# Clean-slate alternative prompt (superseded)

This was the original from-scratch draft built before the client shared the
actual live GHL prompt. It's kept here for reference only — `02-system-prompt.md`
now contains the client's real prompt patched with the gaps found, and is the
one to use. This file is not recommended for use on its own since it doesn't
match the client's existing verification flow, merge-field conventions, or
objection handling.
